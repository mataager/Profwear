// uploadToCloudinary
const uploadPreset = ""; // Replace with your unsigned upload preset name
...
async function imgurUpload(clientId, file) {
  try {
    const formData = new FormData();
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.data?.error || `HTTP error! status: ${response.status}`
      );
    }
